...
- [Пакет выбора текстов для будущего прототипа Лисы](docs/product/analysis/presentation-link-lisa-user-journey/candidate-evidence/owner-selection-packet.md) — по пять кандидатов для подписи кнопки, сообщения о начале формирования, сообщения об успешной отправке, темы и тела письма; до выбора всех пяти текстов визуальные кадры не меняются.
