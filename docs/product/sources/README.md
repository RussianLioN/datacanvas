...
- [Контролируемый XLSX-источник](reference/datacanvas-backlog-source-sanitized.xlsx) — копия Excel-источника пользовательских историй, приоритетов, сроков и оценок без служебного `absPath`. Все остальные части OOXML совпадают с оригиналом.
...
- [Рабочая XLSX-версия с оценкой ПШЕ](working/datacanvas-backlog-draft-pshe-2026-07-08.xlsx) — версия исходного листа `Лист1` с добавленными строками от `DC-ST-23` — пользовательской истории запуска другим агентом — до `DC-ST-33` — пользовательской истории уведомления в Лисе — и принятыми Product Owner значениями ПШЕ.
