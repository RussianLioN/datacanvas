...
- Значимые изменения проектной документации проходят через `DocumentationChangeRequest`, анализ влияния и очередь решений; Done не заявляется при открытых блокирующих решениях.
...
- Capacity, варианты компромисса при reprioritization и Jira custom field mapping остаются открытыми для каждого конкретного изменения, пока пользователь или внешний источник не подтвердит значения.
