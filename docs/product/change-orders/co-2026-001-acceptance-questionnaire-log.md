...
| Продукт 25 | После принятой Excel-редакции ближайший `P1` — приоритет первого контура — формируют `DC-ST-23` - `DC-ST-29`; `DC-ST-09` остается `P2`, а `DC-ST-01` - `DC-ST-08` находятся в `P3` и не входят в ближайший `P1/P2` без отдельного решения Product Owner. | confirmed |
