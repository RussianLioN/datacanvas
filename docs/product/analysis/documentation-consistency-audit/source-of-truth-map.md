...
| `DC-ST-*` — пользовательские истории DataCanvas: `docs/product/requirements/user-stories.md` | Канонический каталог пользовательских историй | active | Содержит `DC-ST-23..DC-ST-33` — принятые истории запуска другим агентом, подготовки результата и отдельного этапа доставки по ссылке; приоритеты и ПШЕ синхронизированы с принятой Excel-редакцией. |
