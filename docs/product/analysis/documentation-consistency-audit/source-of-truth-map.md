...
| `DC-ST-*` — пользовательские истории DataCanvas: `docs/stories.md` | Канонический каталог stories | active | Содержит `DC-ST-23..DC-ST-29` — подтвержденные Product Owner P1-истории запуска другим агентом и подготовки результата; рабочие ПШЕ требуют проверки командой реализации по Excel-матрице трудозатрат и ролей. |
...
| Product backlog — продуктовый список работ: `docs/product/backlog/product-backlog.md` | Product planning | draft | Хранит `DC-ST-23..DC-ST-29` как P1-истории с рабочими значениями ПШЕ в Excel; утвержденный sprint backlog — список работ спринта — ждет проверки команды, полного BA-опросника DataCanvas и решения по вытеснению работ. |
