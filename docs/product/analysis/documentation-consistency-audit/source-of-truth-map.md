...
| `DC-ST-*` — пользовательские истории DataCanvas: `docs/product/requirements/user-stories.md` | Канонический каталог пользовательских историй | active | Содержит `DC-ST-23..DC-ST-29` — принятые истории запуска другим агентом и подготовки результата; приоритеты синхронизированы с принятой Excel-редакцией. |
...
| Traceability — трассировка: `docs/product/requirements/traceability-matrix.json` | Traceability | draft | Связывает `DC-ST-23..DC-ST-29` с `BT-002`, `BT-012`, `BT-014..BT-018`, критериями приемки и будущей проверкой задач. |
