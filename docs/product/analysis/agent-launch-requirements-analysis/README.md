...
Пакет хранит состояние, журнал и проверяемые решения по влиянию `DC-ST-23` — пользовательской истории о передаче запроса от другого агента — через `DC-ST-28` — пользовательскую историю о проверяемости запусков другим агентом — на `BT-*` — бизнес-требования.
...
- `story-requirement-decision-ledger.md` — решения по `DC-ST-23..DC-ST-28` — пользовательским историям-кандидатам.
