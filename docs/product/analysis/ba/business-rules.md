...
| `BRULE-006` — правило безопасного состояния | safe_lisa_status | Пользователь видит в том же чате Лисы только безопасное состояние заказа; личные сведения и содержание запроса не показываются | Security/Privacy Lead | `npm run validate:business-rules` |
