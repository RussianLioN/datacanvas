...
| `BRULE-007` — правило границы Q4_2026 | q4_scope_boundary | В Q4_2026 входят адреса через «Профиль сотрудника», `PPTX`/`PDF`, почта и состояние в том же чате; ссылка, хранилище, PUSH и расширенное редактирование структуры относятся к будущему объему | Product Owner | `npm run validate:business-rules` |
