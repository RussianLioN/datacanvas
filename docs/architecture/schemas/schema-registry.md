...
| JiraStoryImportContract | Договор воспроизводимого CSV с пользовательскими историями и ресурсными оценками для Jira | active: `schemas/jira-story-import-contract.schema.json` |
| JiraImportPackageManifest | Manifest Jira import package readiness | draft: `schemas/jira-import-package-manifest.schema.json` |
| AgentLaunchRequirementsAnalysisState | Состояние анализа `DC-ST-23..DC-ST-29` — пользовательских историй запуска другим агентом — и правило продолжения после обрыва | active: `schemas/agent-launch-requirements-analysis-state.schema.json` |
| AgentLaunchRequirementsImpactMap | Карта влияния `DC-ST-23..DC-ST-29` — пользовательских историй запуска другим агентом — на `BT-*` — бизнес-требования | active: `schemas/agent-launch-requirements-impact-map.schema.json` |
