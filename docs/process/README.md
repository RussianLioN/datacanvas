...
- [План выделения автоматизации проектной документации в отдельный проект](../plans/project-documentation-automation-extraction-implementation-plan.md)
- [`PROC-064` — выделение автоматизации проектной документации](change-requests/PROC-064-project-documentation-automation-extraction.md)
...
Универсальный рабочий процесс документации выделяет переносимое ядро методики, профиль конкретного ИТ-продукта и экземпляр изменения. DataCanvas в нем используется как пилотный профиль; продуктовые решения остаются в продуктовых источниках истины, а правила рабочего процесса живут в домене `documentation_operations`.

`PROC-064` — выделение автоматизации проектной документации — принимает следующий шаг: переносимое ядро, контракты, шаблоны, планировщик и проверки переходят в отдельный закрытый проект. DataCanvas остается пилотом и эталонной реализацией видов артефактов, их связей, форматов и проверок, но его продуктовый смысл не переносится.
