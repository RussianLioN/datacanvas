...
Этот репозиторий является контуром проектной документации и документального сопровождения DataCanvas. Фактическая реализация и разработка продукта ведутся вне этого проекта; локальные scripts, schemas, fixtures и generated artifacts обслуживают документацию, проверки и evidence.
...
## Контракт PO-Опросника

PO-опросник по Product Change Order ведётся как сохраняемый артефакт, а не как только чатовая сессия.

Обязательные правила:

1. Один смысловой вопрос за шаг.
2. После каждого ответа Product Owner обновляются JSON-состояние и Markdown-журнал.
3. После сохранения запускается лёгкая проверка состояния.
4. Каждые 5 ответов выполняется контрольная остановка и показывается checklist продолжения.
5. При возобновлении агент читает state и log, показывает краткое резюме и продолжает с вопроса, записанного в state.
6. Если вопрос касается существующего раздела, агент сначала выводит текущий текст раздела, затем даёт простой комментарий и только после этого задаёт вопрос.
7. Полная методологическая валидация выполняется после завершения всего опросника, а не после каждой промежуточной остановки.

Для `CO-2026-001` активный контракт находится в `docs/product/change-orders/product-change-questionnaire-protocol.md`.
