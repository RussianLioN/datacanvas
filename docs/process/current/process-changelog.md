...
Оригинальный XLSX с закрытыми метаданными заменён в Git на контролируемую копию и машинный манифест происхождения. Добавлены блокирующие проверки содержимого XLSX во всей истории кандидата и точного SHA, проверяемого CI. Принятие рабочей редакции Product Owner отделено от командного согласования трудозатрат: до него перенос в sprint backlog и Jira заблокирован. Проверка каскадного процесса теперь честно объявляет временные записи и обязательную очистку, а временные пути исключены из Git.
