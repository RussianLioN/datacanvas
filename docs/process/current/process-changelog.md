...
Оригинальный XLSX с закрытыми метаданными заменён в Git на контролируемую копию и машинный манифест происхождения. Добавлены блокирующие проверки содержимого XLSX во всей истории кандидата и точного SHA, проверяемого CI.
...
Для SVG, PNG, PDF и PlantUML введена обязательная проверка границ текста, пересечений рамок, равномерности сетки, границ холста и свежести переносимых рендеров. Актуальный BMC-пакет больше не включает старые снимки предыдущей версии как текущее evidence.
