...
Для SVG, PNG, PDF и PlantUML введена обязательная проверка границ текста, пересечений рамок, равномерности сетки, границ холста и свежести переносимых рендеров. Проверка SVG теперь охватывает заголовок, внутренние прямоугольники и разделительные линии; проверка PlantUML связывает каждый номер бизнес-блока с правильной позицией, а не только считает номера. Видимые служебные подписи удалены из визуального BMC, а комментарий генератора — из публичного PlantUML. Актуальный BMC-пакет больше не включает старые снимки предыдущей версии как текущее evidence.
