...
- Плановый пакет содержит безопасный журнал запуска. Финализированный пакет дополнительно фиксирует реестр входов и выходов кандидата, фактический Git diff и единый отпечаток кандидата. Тот же отпечаток обязан совпадать в run record, diff-манифесте, профильном evidence, completion evidence и печати завершения. Проверочные стадии заново выводят ожидаемые `base_sha`, `planning_head_sha`, `candidate_head_sha`, `input_paths`, `output_paths`, `allowed_write_paths` и политику архива из независимых run/resolution/planning/profile источников; сам проверяемый diff-манифест не может сам себе подтвердить Git-якоря, состав или расширить разрешённую область записи.
- Архив не может быть собственным входом при расчете отпечатка кандидата. Любая попытка включить путь архива во входной реестр блокирует публикацию пакета.
- Только `cascade:complete` после профильных проверок, явного completion-профиля, повторного вычисления фактического Git diff и проверки незарегистрированных файлов может выпустить состояние `verified` и печать завершения. Completion-профиль не запускает общий `npm test`; он выполняет перечисленный набор проверок каскада, схем, безопасности, утечек данных, whitespace и чистоты рабочей копии.
...
Пакет содержит идентичность источников, смысловую область влияния, список обязательных проверок, сведения о среде, журнал запуска и, если нужно решение, пакет вопроса владельцам. Все определяющие входы связаны ключом повторяемости. Тот же Git-диапазон и те же входы должны дать тот же ключ.
...
Созданный каталог `<finalization-id>` содержит реестр входов и выходов, фактический diff и отпечаток кандидата. Он фиксируется отдельным коммитом. Затем профильные проверки выполняются в отдельной рабочей копии candidate SHA; зависимости устанавливаются туда через `npm ci --ignore-scripts`, без ссылки на `node_modules` основной рабочей копии, с временными `HOME`, npm cache и пустым npm userconfig:
...
Если любая стадия не прошла, ее новый каталог не публикуется. Ошибка до публикации не должна оставлять частичный пакет, архив, переключение или evidence; в выводе команды остается только безопасная запись журнала с командой, исходом, кодом выхода, краткой причиной, `evidence_path: null` и достижимой ссылкой на RCA либо `rca_path: null`, без закрытых путей и данных.
