...
const blockModel = [
  ...
  },
  {
    ...
    shortTitle: "Ресурсы",
    statement:
      "Ключевые ресурсы DataCanvas: входной контекст, описание презентации, PresentationSpec, шаблоны, renderer, схемы, проверки качества, артефакты каскадного управления и интеграции Лисы/A2A/MCP.",
    bullets: [
      "Входной контекст",
      "Описание и PresentationSpec",
      "Шаблоны презентаций",
      "Renderer и quality gates",
      "Артефакты каскадного управления",
    ...
    detail: [
      ...
      "Dependency graph, отчет анализа влияния, очередь решений, capacity plan и Jira field mapping request создают воспроизводимый путь изменения связанных проектных артефактов.",
    ],
  },
  {
    id: "B7",
    ...
    statement:
      "DataCanvas проверяет достаточность входа, уточняет данные, формирует описание, принимает правки, генерирует и доставляет файл, собирает замечания, ведет каскадный анализ влияния для документации и повторяет цикл.",
    bullets: [
      "Проверка достаточности входа",
      "Уточнение данных",
      "Описание и правки",
      "Генерация и доставка",
      "Каскадный анализ влияния",
    ...
    detail: [
      ...
      "При изменении верхнеуровневого продукта или backlog DataCanvas определяет созависимые документы, фиксирует blocking decisions и не применяет неподтвержденные смысловые правки.",
    ],
  },
  {
    id: "B8",
...
function markdown() {
  ...
  return [
    ...
    "",
    "Проектная документация DataCanvas ведется как управляемый жизненный цикл: изменения Vision, BMC, stories, требований, backlog, roadmap, capacity и Jira import package проходят через анализ влияния, очередь решений и validation evidence. Это не добавляет новых пользовательских обещаний в BMC, но фиксирует обязательный механизм согласованного изменения продуктовой модели.",
    "",
    ...
    "- Машинные артефакты: входной контекст, PresentationSpec, trace, validation companion JSON, визуальный пакет BMC, dependency graph, отчет анализа влияния и очередь решений.",
    "",
    "## Краткая канва",
    "",
