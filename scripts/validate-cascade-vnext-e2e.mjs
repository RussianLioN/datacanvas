...
try {
  ...
  const archiveRefresh = runNodeScript(
    worktree,
  ...
  assert.equal(
    ...
    "подготовительная правка Vision должна менять архив документации и манифест хэшей",
  );
  exec(
    "git",
    [
  ...
  commitTestChange(worktree, "test: refresh derived artifacts after planner delta");
  const archiveCheckAfterCommit = runNodeScript(
    worktree,
    "scripts/generate-documentation-archive.mjs",
    ["--check"],
  );
  assert.equal(
    archiveCheckAfterCommit.status,
    0,
    output(archiveCheckAfterCommit),
  );
