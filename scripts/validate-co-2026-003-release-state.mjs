...
function validateIndependentReleaseDecisions(ledger) {
  if (ledger.release_decisions.draft_archive.release_allowed !== false || ledger.release_decisions.delivery_archive.public_allowed !== true || ledger.release_decisions.publicity.public_allowed !== true) throw new Error("статусы выпуска должны сохранять независимые границы черновика и публичности");
  const finalApproved = ledger.final_release.status === "owner_final_approved";
  if (
    ledger.final_release.active_release_switch_allowed !== finalApproved ||
    ledger.final_release.high_resolution_render_allowed !== finalApproved ||
    ledger.final_release.delivery_archive_allowed !== finalApproved ||
    ledger.release_decisions.delivery_archive.creation_allowed !== finalApproved ||
    ledger.release_decisions.active_release_switch.switch_allowed !== finalApproved ||
    ledger.release_decisions.high_resolution_render.render_allowed !== finalApproved ||
    ledger.release_decisions.high_resolution_render.current_render_allowed !== finalApproved
